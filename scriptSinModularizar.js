...
// PARA ORDENAR UN ARRAY (ESTA SECCiÓN ES PARA LA ENTREGA DE "ORDENAR UN ARRAY" ):
// TODAVIA NO TERMINADO
/*
function encontrarIndiceMenor(objectDatabase) {
    const arr = objectDatabase
    let menor = Object.keys(arr[0]);
    let indiceMenor = 0;

    for (let i = 1; i < arr.length; i++) {
        let id = Object.keys(arr[i])
        if (id < menor) {
            menor = id;
            indiceMenor = i;
        }
...
function selectionSortAsc(objectDatabase) {
    const arr = objectDatabase
    const nuevoArr = [];
